...
class Tasks extends Berp3Api
{
	...
	public function get($id, $includetimespent = 0)
	{
		...
		if (!Berp3Api::_checkAccessToResource('task', $this->task->id)) {
	...
	public function post($request_data = null)
	{
		...
		if ($this->task->create(Berp3ApiAccess::$user) < 0) {
	...
	*/
	...
	public function getRoles($id, $userid = 0)
	{
		...
		if (!Berp3Api::_checkAccessToResource('tasks', $this->task->id)) {
			throw new RestException(401, 'Access not allowed for login '.Berp3ApiAccess::$user->login);
		}

		$usert = Berp3ApiAccess::$user;
		if ($userid > 0) {
	...
	*/
	...
	public function put($id, $request_data = null)
	{
		...
		if ($this->task->update(Berp3ApiAccess::$user) > 0) {
			return $this->get($id);
		} else {
	...
	public function delete($id)
	{
		...
		if (!$this->task->delete(Berp3ApiAccess::$user)) {
	...
	public function addTimeSpent($id, $date, $duration, $user_id = 0, $note = '')
	{
		...
		if (!Berp3Api::_checkAccessToResource('project', $this->task->fk_project)) {
			throw new RestException(401, 'Access not allowed for login '.Berp3ApiAccess::$user->login);
		}

		$uid = $user_id;
		if (empty($uid)) {
			$uid = Berp3ApiAccess::$user->id;
		}
		...
		$result = $this->task->addTimeSpent(Berp3ApiAccess::$user, 0);
		if ($result == 0) {
