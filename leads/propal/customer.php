...
$lead  = new Leads($db);
$leadid = (!empty(GETPOST('leads'))) ? GETPOST('leads') : 0 ;
$leads = $lead->fecthLeadsById($leadid);
//echo $lead->rowid;
...
if (empty($reshook)) {
	$backurlforlist = DOL_URL_ROOT.'/leads/propal/card.php?mainmenu=commercial&action=view&leftmenu=leads&leads='.$lead->rowid;
			...
			if ($action == 'add') {
				...
				if ($result >= 0 && !$error) {
					...
					if (!empty($backtopage)) {
						...
						//$lead->progress = 'convertedtoquotation';
						$lead->rowid = (!empty(GETPOST('leads'))) ? GETPOST('leads') : 0 ;
						//$lead->updateProgress();
						$url = DOL_URL_ROOT."/leads/propal/quotation.php?mainmenu=commercial&action=add&leftmenu=propals&socid=".$object->id.'&leads='.$lead->rowid;
						//echo $leadid;
						header("Location: ".$url);
						exit;
					} else {
...
if (is_object($objcanvas) && $objcanvas->displayCanvasExists($action)) {
	...
	if ($action == 'create') {
        ...
        //$object->rowid;
        $object->name = $lead->ref_client;
        $object->name_alias = $lead->ref_client;
        $object->phone = $lead->phone;
        $object->email = $lead->email;
		...
		print '<input type="hidden" name="leads" value="'.$lead->rowid.'">';
		...
		$selected = (count(GETPOST('commercial', 'array')) > 0 ? GETPOST('commercial', 'array') : (GETPOST('commercial', 'int') > 0 ? array(GETPOST('commercial', 'int')) : (empty($user->rights->societe->client->voir) ? array($user->id) : array($lead->sales_person_id))));
