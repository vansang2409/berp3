...
$object->ref_client = $societe->name;
$object->socid = $societe->id;
...
if (empty($reshook)) {
	...
	if ($action == 'confirm_clone' && $confirm == 'yes' && $usercancreate) {
		...
		$object->socid = $socid;
		$object->fetch_thirdparty();
        $day = (int)(GETPOST('reday') ? GETPOST('reday') : date('d'));
        $month =(int)(GETPOST('remonth') ? GETPOST('remonth') : date('m'));
        $year = (int)(GETPOST('reyear') ? GETPOST('reyear') : date('Y'));
		$datep = dol_mktime(12, 0, 0,$month,$day,$year);
		$date_delivery = dol_mktime(12, 0, 0, GETPOST('date_livraisonmonth'), GETPOST('date_livraisonday'), GETPOST('date_livraisonyear'));
		 $duration = (int) (GETPOST('duree_validite', 'int') ? GETPOST('duree_validite', 'int'): 15);
		...
		if (!$error) {
			...
			if (GETPOST('createmode') == 'copy' && GETPOST('copie_propal')) {
				...
				$object->ref = GETPOST('ref');
				$object->ref_client = (!empty(GETPOST('ref_client')) ? GETPOST('ref_client'):$societe->name);
				$object->datep = $datep;
				$object->date = $datep;
				$object->date_livraison = $date_delivery;
				$object->delivery_date = $date_delivery;
				...
				$object->shipping_method_id = GETPOST('shipping_method_id', 'int');
				$object->warehouse_id = GETPOST('warehouse_id', 'int');
				$object->duree_validite = $duration;//price2num(GETPOST('duree_validite', 'alpha'));
...
if ($action == 'create') {
	...
	print '<input type="text" name="ref_client" class="width100p" value="'.(GETPOST('ref_client') ? GETPOST('ref_client') : $object->ref_client).'"></td>';
	print '</tr>';
	...
	if ($socid > 0) {
		...
		print '<td>';
		print img_picto('', 'company').$form->select_company($object->socid, 'socid', '(s.client = 1 OR s.client = 2 OR s.client = 3) AND status=1', 'SelectCustomer', 0, 0, null, 0, 'minwidth300  widthcentpercentminusxx');
