...
class modEmailCollector extends Berp3Modules
{
	...
	public function init($options = '')
	{
		...
		if ($tmpresql) {
			if ($this->db->num_rows($tmpresql) == 0) {
				...
				$sqlforexampleB1 .= " VALUES (".$conf->entity.", 'Collect_Responses_In', 'Example to collect any received email that is a response of an email sent from BERP3', '".$this->db->escape($descriptionB1)."', 'INBOX', '".$this->db->idate(dol_now())."', ".((int) $user->id).", 0)";
	 ...
	 *	Remove from database constants, boxes and permissions from BERP3 database.
