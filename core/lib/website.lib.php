...
function dolWebsiteReplacementOfLinks($website, $content, $removephppart = 0, $contenttype = 'html', $containerid = '')
{
	...
	dol_syslog('dolWebsiteReplacementOfLinks start (contenttype='.$contenttype." containerid=".$containerid." USEBERP3EDITOR=".(defined('USEBERP3EDITOR') ? '1' : '')." USEBERP3SERVER=".(defined('USEBERP3SERVER') ? '1' : '').')', LOG_DEBUG);
	...
	// Replace relative link '/' with berp3 URL
	...
	// Replace relative link /xxx.php#aaa or /xxx.php with berp3 URL (we discard param ?...)
	$content = preg_replace('/(href=")\/?([^:\"\!]*)\.php(#[^\"<>]*)?\"/', '\1!~!~!~'.DOL_URL_ROOT.'/website/index.php?website='.$website->ref.'&pageref=\2\3"', $content, -1, $nbrep);
	// Replace relative link /xxx.php?a=b&c=d#aaa or /xxx.php?a=b&c=d with berp3 URL
	...
	// <img src="medias/...image.png... => <img src="berp3/viewimage.php/modulepart=medias&file=image.png...
	// <img src="...image.png... => <img src="berp3/viewimage.php/modulepart=medias&file=image.png...
	...
	// <img src="viewimage.php/modulepart=medias&file=image.png" => <img src="berp3/viewimage.php/modulepart=medias&file=image.png"
	$content = preg_replace('/(<img[^>]*src=")(\/?viewimage\.php)/', '\1!~!~!~'.DOL_URL_ROOT.'/viewimage.php', $content, -1, $nbrep);

	// action="newpage.php" => action="berp3/website/index.php?website=...&pageref=newpage
 ...
 * Used to ouput the page when viewed from a server (Berp3 or Apache).
 *
...
function dolWebsiteOutput($content, $contenttype = 'html', $containerid = '')
{
	...
	global $berp3_main_url_root, $berp3_main_data_root;
	global $website;
	global $includehtmlcontentopened;

	$nbrep = 0;

	dol_syslog("dolWebsiteOutput start - contenttype=".$contenttype." containerid=".$containerid." USEBERP3EDITOR=".(defined('USEBERP3EDITOR') ? '1' : '')." USEBERP3SERVER=".(defined('USEBERP3SERVER') ? '1' : '').' includehtmlcontentopened='.$includehtmlcontentopened);
	...
	if (defined('USEBERP3EDITOR')) {		// REPLACEMENT OF LINKS When page called from BERP3 editor
	...
	} elseif (defined('USEBERP3SERVER')) {	// REPLACEMENT OF LINKS When page called from BERP3 server
		...
		// Replace relative link / with berp3 URL:  ...href="/"...
		$content = preg_replace('/(href=")\/\"/', '\1!~!~!~'.DOL_URL_ROOT.'/public/website/index.php?website='.$website->ref.'"', $content, -1, $nbrep);
		// Replace relative link /xxx.php#aaa or /xxx.php with berp3 URL:  ...href="....php" (we discard param ?...)
		...
		// Replace relative link without .php like /xxx#aaa or /xxx with berp3 URL:  ...href="....php"
		...
		// Fix relative link /document.php with correct URL after the DOL_URL_ROOT:  href="/document.php?modulepart=" => href="/berp3/document.php?modulepart="
		$content = preg_replace('/(href=")(\/?document\.php\?[^\"]*modulepart=[^\"]*)(\")/', '\1!~!~!~'.DOL_URL_ROOT.'\2\3', $content, -1, $nbrep);
		$content = preg_replace('/(src=")(\/?document\.php\?[^\"]*modulepart=[^\"]*)(\")/', '\1!~!~!~'.DOL_URL_ROOT.'\2\3', $content, -1, $nbrep);

		// Fix relative link /viewimage.php with correct URL after the DOL_URL_ROOT: href="/viewimage.php?modulepart=" => href="/berp3/viewimage.php?modulepart="
...
function dolWebsiteSaveContent($content)
{
	...
	//dol_syslog("dolWebsiteSaveContent start (mode=".(defined('USEBERP3SERVER')?'USEBERP3SERVER':'').')');
...
function redirectToContainer($containerref, $containeraliasalt = '', $containerid = 0, $permanent = 0)
{
	...
	if (defined('USEBERP3SERVER')) {	// When page called from BERP3 server
...
function getSocialNetworkSharingLinks()
{
	...
	if ($website->virtualhost) {
		...
		$out .= '<script>(function(d, s, id) {
				  ...
				  js.src = "//connect.facebook.net/en_US/sdk.js#xfbml=1&version=v2.0&amp;appId=berp3.org";
...
function getPagesFromSearchCriterias($type, $algo, $searchstring, $max = 25, $sortfield = 'date_creation', $sortorder = 'DESC', $langcode = '', $otherfilters = 'null', $status = 1)
{
	...
	if (!$error && (empty($max) || ($found < $max)) && (preg_match('/sitefiles/', $algo))) {
		...
		$pathofwebsite = $berp3_main_data_root.'/website/'.$website->ref;
