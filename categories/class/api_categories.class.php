 ...
 */
class Categories extends Berp3Api
{
	/**
	...
	public function get($id, $include_childs = false)
	{
		...
		if (!Berp3Api::_checkAccessToResource('categorie', $this->category->id)) {
	...
	{
		if (!Berp3ApiAccess::$user->rights->categorie->creer) {
			throw new RestException(401);
		}
		...
		if ($this->category->create(Berp3ApiAccess::$user) < 0) {
	...
	public function put($id, $request_data = null)
	{
		...
		if ($this->category->update(Berp3ApiAccess::$user) > 0) {
			return $this->get($id);
		} else {
	...
	{
		if (!Berp3ApiAccess::$user->rights->categorie->supprimer) {
			throw new RestException(401);
		}
		$result = $this->category->fetch($id);
		if (!$result) {
		...
		if (!$this->category->delete(Berp3ApiAccess::$user)) {
	...
	public function getListForObject($id, $type, $sortfield = "s.rowid", $sortorder = 'ASC', $limit = 0, $page = 0)
	{
		...
		if ($type == Categorie::TYPE_PRODUCT && !(Berp3ApiAccess::$user->rights->produit->lire || Berp3ApiAccess::$user->rights->service->lire)) {
			throw new RestException(401);
		} elseif ($type == Categorie::TYPE_CONTACT && !Berp3ApiAccess::$user->rights->contact->lire) {
			throw new RestException(401);
		} elseif ($type == Categorie::TYPE_CUSTOMER && !Berp3ApiAccess::$user->rights->societe->lire) {
			throw new RestException(401);
		} elseif ($type == Categorie::TYPE_SUPPLIER && !Berp3ApiAccess::$user->rights->fournisseur->lire) {
			throw new RestException(401);
		} elseif ($type == Categorie::TYPE_MEMBER && !Berp3ApiAccess::$user->rights->adherent->lire) {
			throw new RestException(401);
		} elseif ($type == Categorie::TYPE_PROJECT && !Berp3ApiAccess::$user->rights->projet->lire) {
			throw new RestException(401);
		}
	...
	public function linkObjectById($id, $type, $object_id)
	{
		...
		if ($type === Categorie::TYPE_PRODUCT) {
			if (!(Berp3ApiAccess::$user->rights->produit->creer || Berp3ApiAccess::$user->rights->service->creer)) {
				throw new RestException(401);
			}
