...
if (!defined('DONOTLOADCONF') && file_exists($conffile) && filesize($conffile) > 8) { // Test on filesize is to ensure that conf file is more that an empty template with just <?php in first line
	...
	if ($result) {
		if (empty($berp3_main_db_type)) {
			$berp3_main_db_type = 'mysql'; // For backward compatibility
		}
		...
		// Clean parameters
		$berp3_main_data_root        = isset($berp3_main_data_root) ?trim($berp3_main_data_root) : '';
...
define('DOL_DATA_ROOT', (isset($berp3_main_data_root) ? $berp3_main_data_root : ''));
...
if (empty($berp3_main_db_character_set)) {
	$berp3_main_db_character_set = ($conf->db->type == 'mysqli' ? 'utf8' : ''); // Old installation
}
$conf->db->character_set = $berp3_main_db_character_set;
if (empty($berp3_main_db_collation)) {
	$berp3_main_db_collation = ($conf->db->type == 'mysqli' ? 'utf8_unicode_ci' : ''); // Old installation
}
...
function pHeader($soutitre, $next, $action = 'none')
{
	...
	print '<meta name="keywords" content="help, center, berp3, berp3wamp">'."\n";
	print '<meta name="description" content="Berp3 help center">'."\n";
	print '<link rel="stylesheet" type="text/css" href="../install/default.css">'."\n";
	print '<title>'.$langs->trans("Berp3HelpCenter").'</title>'."\n";
	print '</head>'."\n";
