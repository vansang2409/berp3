...
if (!class_exists('PrestaShopWebservice')) { // We keep this because some modules add this lib too into a different path. This is to avoid "Cannot declare class PrestaShopWebservice" errors.
	include_once DOL_DOCUMENT_ROOT.'/admin/berp3store/class/PSWebServiceLibrary.class.php';
}
...
class Berp3store
{
	...
	public function __construct($debug = false)
	{
		...
		$this->shop_url  = 'https://www.berp3store.com/index.php?controller=product&id_product=';
	...
	/**
	 * Load data from remote Berp3store market place.
	...
	public function getRemoteCategories()
	{
		...
		try {
			$this->api = new PrestaShopWebservice($conf->global->MAIN_MODULE_BERPSTORE_API_SRV, $conf->global->MAIN_MODULE_BERPSTORE_API_KEY, $this->debug_api);
			dol_syslog("Call API with MAIN_MODULE_BERPSTORE_API_SRV = ".getDolGlobalString('MAIN_MODULE_BERPSTORE_API_SRV'));
			// $conf->global->MAIN_MODULE_BERPSTORE_API_KEY is for the login of basic auth. There is no password as it is public data.
			...
			if ($trace[0]['args'][0] == 404) {
				...
				print 'Can not access to '.$conf->global->MAIN_MODULE_BERPSTORE_API_SRV.'<br>';
				print $e->getMessage();
			}
		}
	}
	...
	public function getRemoteProducts($options = array('start' => 0, 'end' => 10, 'per_page' => 50, 'categorie' => 0, 'search' => ''))
	{
		...
		try {
			...
			if ($this->search != '') {
				$opt2['url'] = $conf->global->MAIN_MODULE_BERPSTORE_API_SRV.'/api/search?query='.$this->search.'&language='.$this->lang; // It seems for search, key start with
			...
			$opt['display']        = '[id,name,id_default_image,id_category_default,reference,price,condition,show_price,date_add,date_upd,description_short,description,module_version,berp3_min,berp3_max]';
			$opt['sort']           = 'id_desc';
			$opt['filter[active]'] = '[1]';
	...
	/**
	 * Return tree of Berp3store categories. $this->categories must have been loaded before.
	 *
	...
	public function get_products()
	{
		...
		foreach ($this->products as $product) {
			...
			if ($product->id_default_image != '') {
				$image_url = DOL_URL_ROOT.'/admin/berp3store/ajax/image.php?id_product='.((int) $product->id).'&id_image='.((int) $product->id_default_image);
				...
				$images = '<img src="'.DOL_URL_ROOT.'/admin/berp3store/img/NoImageAvailable.png" />';
			}

			// free or pay ?
			if ($product->price > 0) {
				$price = '<h3>'.price(price2num($product->price, 'MT'), 0, $langs, 1, -1, -1, 'EUR').' '.$langs->trans("HT").'</h3>';
				$download_link = '<a target="_blank" href="'.urlencode($this->shop_url.$product->id).'"><img width="32" src="'.DOL_URL_ROOT.'/admin/berp3store/img/follow.png" /></a>';
			} else {
				$price         = '<h3>'.$langs->trans('Free').'</h3>';
				$download_link = '<a target="_blank" href="'.urlencode($this->shop_url.$product->id).'"><img width="32" src="'.DOL_URL_ROOT.'/admin/berp3store/img/Download-128.png" /></a>';
				$download_link .= '<br><br><a target="_blank" href="'.urlencode($this->shop_url.$product->id).'"><img width="32" src="'.DOL_URL_ROOT.'/admin/berp3store/img/follow.png" /></a>';
			}

			//checking versions
			if ($this->version_compare($product->berp3_min, DOL_VERSION) <= 0) {
				if ($this->version_compare($product->berp3_max, DOL_VERSION) >= 0) {
					//compatible
