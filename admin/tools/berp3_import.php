...
/**
 *		\file 		htdocs/admin/tools/berp3_import.php
 *		\ingroup	core
...
<?php
print '<span class="opacitymedium">';
print $langs->trans("RestoreDesc3", $berp3_main_db_name).'<br><br>';
print '</span>';
?>

<?php print $langs->trans("DatabaseName").' : <b>'.$berp3_main_db_name.'</b>'; ?><br><br>
...
if (in_array($type, array('mysql', 'mysqli'))) {
	...
	$param = $berp3_main_db_name;
	$param .= " -h ".$berp3_main_db_host;
	...
	if (!empty($berp3_main_db_pass)) {
		...
		$paramclear .= " -p".$berp3_main_db_pass;
	}
	...
	if (empty($_GET["showpass"]) && $berp3_main_db_pass) {
	...
	print ajax_autoselect('restorecommand');
	//if (empty($_GET["showpass"]) && $berp3_main_db_pass) print '<br><a href="'.$_SERVER["PHP_SELF"].'?showpass=1&amp;radio_dump=postgresql_options">'.$langs->trans("UnHidePassword").'</a>';
	//else print '<br><a href="'.$_SERVER["PHP_SELF"].'?showpass=0&amp;radio_dump=mysql_options">'.$langs->trans("HidePassword").'</a>';
	print '</div>';
