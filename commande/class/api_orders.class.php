...
class Orders extends Berp3Api
{
	...
	private function _fetch($id, $ref = '', $ref_ext = '', $contact_list = 1)
	{
		...
		if (!Berp3Api::_checkAccessToResource('commande', $this->commande->id)) {
	...
	{
		if (!Berp3ApiAccess::$user->rights->commande->creer) {
		...
		if ($this->commande->create(Berp3ApiAccess::$user) < 0) {
	...
	public function deleteLine($id, $lineid)
	{
		...
		$updateRes = $this->commande->deleteline(Berp3ApiAccess::$user, $lineid);
		if ($updateRes > 0) {
			return $this->get($id);
		} else {
	...
	public function put($id, $request_data = null)
	{
		...
		if ($this->commande->update(Berp3ApiAccess::$user) > 0) {
			return $this->get($id);
		} else {
	...
	{
		if (!Berp3ApiAccess::$user->rights->commande->supprimer) {
			throw new RestException(401);
		}
		$result = $this->commande->fetch($id);
		if (!$result) {
		...
		if (!$this->commande->delete(Berp3ApiAccess::$user)) {
	...
	public function validate($id, $idwarehouse = 0, $notrigger = 0)
	{
		...
		$result = $this->commande->valid(Berp3ApiAccess::$user, $idwarehouse, $notrigger);
		if ($result == 0) {
	...
	public function reopen($id)
	{
		...
		$result = $this->commande->set_reopen(Berp3ApiAccess::$user);
		if ($result < 0) {
			throw new RestException(405, $this->commande->error);
		} elseif ($result == 0) {
	...
	public function setinvoiced($id)
	{
		...
		$result = $this->commande->classifyBilled(Berp3ApiAccess::$user);
		if ($result < 0) {
			throw new RestException(400, $this->commande->error);
		}
	...
	public function close($id, $notrigger = 0)
	{
		...
		$result = $this->commande->cloture(Berp3ApiAccess::$user, $notrigger);
		if ($result == 0) {
	...
	public function settodraft($id, $idwarehouse = -1)
	{
		...
		$result = $this->commande->setDraft(Berp3ApiAccess::$user, $idwarehouse);
		if ($result == 0) {
	...
	public function createOrderFromProposal($proposalid)
	{
		...
		$result = $this->commande->createFromProposal($propal, Berp3ApiAccess::$user);
		if ($result < 0) {
			throw new RestException(405, $this->commande->error);
		}
		$this->commande->fetchObjectLinked();
	...
	{
		require_once DOL_DOCUMENT_ROOT.'/expedition/class/expedition.class.php';
		if (!Berp3ApiAccess::$user->rights->expedition->creer) {
			throw new RestException(401);
		}
		if ($warehouse_id <= 0) {
		...
		$result = $shipment->create(Berp3ApiAccess::$user);
		if ($result <= 0) {
